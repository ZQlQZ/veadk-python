...
**Agent 概述**： 语意上，智能体是电子世界里的一个智能虚拟代理人。它以自然语言作为交互方式，接收外部指令，对输入进行感知，继而分析和决策，并采取相应行动。同时它具备获取专业知识和记录自己行为记忆以辅助分析和决策的能力。工程上，智能体通过大语言模型实现对多模态输入进行识别，分析，决策，过程中通过 MCP，工具，API，Web Search访问外部系统（比如获取天气信息）和通过访问知识库获取更专业领域的输入，并通过大模型进行分析和决策后，通过自然语言进行输出，或直接使用相关工具进程处理，比如订票。除此之外，Agent 可以通过 A2A协议和其他 Agent进行交互，组建更强大的能力。同时  Agent具有自我学习和演化能力，逐步提升自己。总的来说，Agent是未来人/机器与机器/服务交互的方式。
...
| **存储工具** | OpenSearch、Viking DB、FAISS（本地轻量向量库）。 |
